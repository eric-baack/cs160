...
class Graph:
    ...
    def hub(self):
        ...
        for vert in self:
            elist = {}
            for nvert in vert.all_neighbors:
                estr = vert.key + nvert.key
                if estr not in elist:  # This is probably un-necessary
                    elist[estr] = 1
            if len(elist) > maxedges:
                maxedges = len(elist)
                hubstr = vert.key
        return hubstr
    ...
    def size(self):
        ...
        for vert in self:
            for nvert in vert.all_neighbors:
                estr = vert.key + nvert.key
                investr = nvert.key + vert.key
                if investr not in elist:
                    if estr not in elist:  # necessary?
                        elist[estr] = 1
        return len(elist)
...
def main():
    ...
    G1.add_edge("A", "C", 8)
    print(G1.vertices)
    print(f"len {G1.__len__()}")
    print(f"size {G1.size()}")
    print(f"hub {G1.hub()}")
